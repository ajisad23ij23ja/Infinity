...
dsc.gg/unlimitedness | version {VERSION} | By X2Poor#0001
"""
...
└───────────────────────────────────────────────────────────────────────────────────────┘
┌─Other Tools────────┐                           ┌─other────────┐     ┌─support─────────┐
│  [ d ] Discord     │                           │  [ 0 ] Exit  │     │  [ C ] Credits  │
│  [ s ] Source Code │                           └──────────────┘     └─────────────────┘
└────────────────────┘                                                                   
...
try:
    if authentication['bool'] == False:
        ...
        log("You can buy a license @ dsc.gg/unlimitedness")
        close()
except Exception:
    pass
...
try:
    upd = requests.get("https://github.com/XPTGR/Infinity/blob/main/data.json")
    ...
    log("Please join the Discord (dsc.gg/unlimitedness) and report this.")
    pause()
...
try:
    ...
    fatal("Infinity could not find 'config.json'!")
    close()
ok("Config loaded!")
...
if CONFIG["checkCookiesOnStart"] == True: # CHECK COOKIES ON START
    try:
        ...
        fatal("Infinity could not find your cookies file!")
        close()
...
try:
    ...
    fatal("Infinity could not find your cookies or proxies file!")
    close()
